...
CREATE TABLE school_period (period_id INT NOT NULL AUTO_INCREMENT, period_name VARCHAR(50) NOT NULL, start_date DATE DEFAULT NULL, end_date DATE DEFAULT NULL, period_status VARCHAR(20) NOT NULL CHECK (period_status IN ('Active', 'Concluded', 'Upcoming')), active_period_guard INT GENERATED ALWAYS AS (CASE WHEN period_status = 'Active' THEN 1 END), PRIMARY KEY (period_id), CONSTRAINT uq_school_period_active UNIQUE (active_period_guard));
CREATE TABLE practice_group (group_id INT NOT NULL AUTO_INCREMENT, section VARCHAR(50) NOT NULL, period_id INT NOT NULL, professor_id INT NOT NULL, PRIMARY KEY (group_id), CONSTRAINT uq_group_period_section UNIQUE (period_id, section), CONSTRAINT fk_group_period FOREIGN KEY (period_id) REFERENCES school_period (period_id) ON DELETE CASCADE, CONSTRAINT fk_group_professor FOREIGN KEY (professor_id) REFERENCES professor (professor_id) ON DELETE CASCADE);
CREATE TABLE practitioner (practitioner_id INT NOT NULL, indigenous_language VARCHAR(100) DEFAULT NULL, grade DECIMAL(5, 2) DEFAULT NULL, PRIMARY KEY (practitioner_id), CONSTRAINT fk_practitioner_user FOREIGN KEY (practitioner_id) REFERENCES user (user_id) ON DELETE CASCADE);
CREATE TABLE group_enrollment (enrollment_id INT NOT NULL AUTO_INCREMENT, practitioner_id INT NOT NULL, group_id INT NOT NULL, period_id INT NOT NULL, opportunity_number TINYINT NOT NULL DEFAULT 1 CHECK (opportunity_number BETWEEN 1 AND 2), status VARCHAR(20) NOT NULL DEFAULT 'Active' CHECK (status IN ('Active', 'Inactive')), enrollment_date DATETIME DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (enrollment_id), CONSTRAINT uq_enrollment_practitioner_period UNIQUE (practitioner_id, period_id), CONSTRAINT fk_enrollment_practitioner FOREIGN KEY (practitioner_id) REFERENCES practitioner (practitioner_id) ON DELETE CASCADE, CONSTRAINT fk_enrollment_group FOREIGN KEY (group_id) REFERENCES practice_group (group_id) ON DELETE CASCADE, CONSTRAINT fk_enrollment_period FOREIGN KEY (period_id) REFERENCES school_period (period_id) ON DELETE CASCADE);
...
CREATE TABLE monthly_report (report_id INT NOT NULL AUTO_INCREMENT, practitioner_id INT NOT NULL, month_name VARCHAR(20) NOT NULL, year INT NOT NULL, start_date DATE NOT NULL, end_date DATE NOT NULL, grade DECIMAL(5, 2) DEFAULT NULL, professor_feedback TEXT, status VARCHAR(50) DEFAULT 'Borrador', signed_file_url VARCHAR(500) DEFAULT NULL, PRIMARY KEY (report_id), CONSTRAINT uq_monthly_report_practitioner_month UNIQUE (practitioner_id, year, month_name), CONSTRAINT fk_monthly_report_practitioner FOREIGN KEY (practitioner_id) REFERENCES practitioner (practitioner_id) ON DELETE CASCADE);
CREATE TABLE activity (activity_id INT NOT NULL AUTO_INCREMENT, practitioner_id INT NOT NULL, report_id INT DEFAULT NULL, title VARCHAR(255) NOT NULL, description TEXT, start_date DATE NOT NULL, end_date DATE NOT NULL, duration_hours INT DEFAULT '0', PRIMARY KEY (activity_id), CONSTRAINT fk_activity_practitioner FOREIGN KEY (practitioner_id) REFERENCES practitioner (practitioner_id) ON DELETE CASCADE, CONSTRAINT fk_activity_report FOREIGN KEY (report_id) REFERENCES monthly_report (report_id) ON DELETE SET NULL);
...
CREATE TABLE progress_report (report_id INT NOT NULL AUTO_INCREMENT, practitioner_id INT NOT NULL, report_type VARCHAR(20) NOT NULL, generation_date DATE NOT NULL, period_covered_start DATE NOT NULL, period_covered_end DATE NOT NULL, total_hours_at_submission DECIMAL(6,2) NOT NULL, status VARCHAR(50) DEFAULT 'Pendiente de Firma', signed_file_url VARCHAR(500) DEFAULT NULL, grade DECIMAL(5,2) DEFAULT NULL, professor_feedback TEXT, PRIMARY KEY (report_id), CONSTRAINT uq_progress_report_practitioner_type UNIQUE (practitioner_id, report_type), CONSTRAINT fk_progress_practitioner FOREIGN KEY (practitioner_id) REFERENCES practitioner (practitioner_id) ON DELETE CASCADE);
