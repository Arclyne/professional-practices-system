...
CREATE TABLE practitioner_document (document_id INT NOT NULL AUTO_INCREMENT, practitioner_id INT NOT NULL, document_name VARCHAR(255) NOT NULL, document_type VARCHAR(20) NOT NULL DEFAULT 'Inicial', stored_file_url VARCHAR(500) NOT NULL, status VARCHAR(50) NOT NULL DEFAULT 'Pendiente', review_comment VARCHAR(500) DEFAULT NULL, upload_date DATETIME DEFAULT CURRENT_TIMESTAMP, review_date DATETIME DEFAULT NULL, PRIMARY KEY (document_id), CONSTRAINT fk_document_practitioner FOREIGN KEY (practitioner_id) REFERENCES practitioner (practitioner_id) ON DELETE CASCADE);
