...
public class MessageManager {
    ...
    public boolean sendMessage(int senderId, String receiverEmail, String subject, String body) throws ManagerException {
        ...
        try {
            ...
            if (messageIdentifier > 0) {
                isMessageSent = messageDAO.insertParticipant(messageIdentifier, senderId, receiverId);
            }

        } catch (DAOException e) {
            log.error("Fallo al registrar el mensaje en la base de datos", e);
            throw new ManagerException("No fue posible procesar el envío del mensaje en el sistema.", e);
        }
    ...
    public List<Message> getInboxMessages(int receiverId, int limit, int offset) throws ManagerException {
        ...
        try {
            messagesList = messageDAO.getMessagesByReceiver(receiverId, limit, offset);
        } catch (DAOException e) {
    ...
    public List<Message> getSentMessages(int senderId, int limit, int offset) throws ManagerException {
        ...
        try {
            messagesList = messageDAO.getMessagesBySender(senderId, limit, offset);
        } catch (DAOException e) {
